...
def read_icustays(filepath):
    ...
    icustays["ArriveDTS"] = pd.to_datetime(icustays["ArriveDTS"], errors="coerce")
    icustays["DischargeDTS"] = pd.to_datetime(icustays["DischargeDTS"], errors="coerce")
    icustays["time_diff"] = icustays["DischargeDTS"] - icustays["ArriveDTS"]
    return icustays
...
data_dir = "/N/project/waveform_mortality/xiang/personalized_hemodynamics/data/processed/iuh/"
dst_dir = "/N/project/waveform_mortality/xiang/personalized_hemodynamics/results/barplots/iuh/icustays"
os.makedirs(dst_dir, exist_ok=True)

icustays = read_icustays(os.path.join(data_dir, "demographics.csv"))
...
while time <= 96:
    ...
    windowed_icustays = windowed_icustays.drop_duplicates(["MRN", "FIN"])
    n_expired = (windowed_icustays["Deceased"].notna()).sum()
    n_survival = (windowed_icustays["Deceased"].isna()).sum()
