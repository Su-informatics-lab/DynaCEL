...
def train_model_and_cal_auc(events, y_col="mortality"):
    ...
    model.fit(cp.array(X_train_smote), y_train_smote)
...
for k in range(K):
    ...
    model, auc, bacc, y_pred, y_pred_proba = train_model_and_cal_auc(
    ...
    #     print(i, [feature_names[index], model.feature_importances_[index]])
    #     if i > 5:
    #         break

    model_path = osp.join(
        model_dir, f"XGBoost__PW_{PW}__MOP_{MOP}__OW_{OW}__Fold_{k}.pkl"
    )
    ...
    auc_df.to_csv(
        osp.join(performance_dir, f"XGBoost_AUCs__PW_{PW}__MOP_{MOP}__OW_{OW}.csv"),
        index=False,
    )
    ...
    pred_df.to_csv(
        osp.join(
            pred_dir, f"XGBoost_predictions__PW_{PW}__MOP_{MOP}__OW_{OW}__Fold_{k}.csv"
        ),
    )
