...
output_dir = f"/N/project/waveform_mortality/xiang/personalized_hemodynamics/results/predictions/mimic-iv/predicted_optimals"
...
measurements_median = pd.read_csv(os.path.join(data_dir, f"icustays_model_data_median_PW_{prediction_window}__MOP_{mop}__OW_{outcome_window}.csv"))

n_folds = 5
models = []
for fold in range(n_folds):
    with open(os.path.join(model_dir, f"model__PW_{prediction_window}__MOP_{mop}__OW_{outcome_window}__Fold_{fold}.pkl"), "rb") as model_file:
...
for case_id in list(batched_cross_measurements["stay_id"].unique()):
    measurements_of_interest = merged_measurements.loc[merged_measurements['stay_id'] == case_id, :]
    hr = measurements_of_interest["m1_valuenum"].median()
    bp = measurements_of_interest["m2_valuenum"].median()

    case_df = batched_cross_measurements[batched_cross_measurements['stay_id'] == case_id]
    average_x, average_y = get_average_target_point(case_df, 100)

    actual_risks = []
    optimal_risks = []
    for model_i, model in enumerate(models):
        actual_data = merged_measurements[merged_measurements["stay_id"]==case_id]
        Y = model.predict_proba(actual_data[feature_names])
        actual_risks.append(Y[:, 1])

        optimal_data = merged_measurements[merged_measurements["stay_id"]==case_id].copy(True)
    ...
    result_df = pd.DataFrame({
        ...
        'actual_risk': [np.mean(actual_risks)],
        'optimal_risk': [np.mean(optimal_risks)]
    })
...
final_result_df = final_result_df.merge(
    measurements_median[['stay_id', 'death_in_24_hours']], left_on='case_id', right_on='stay_id', how='left')

# Drop the stay_id column after merge if not needed
final_result_df.drop(columns=['stay_id'], inplace=True)


output_path = os.path.join(output_dir, f"predict_optimal_batch_{batch_idx}.csv")
