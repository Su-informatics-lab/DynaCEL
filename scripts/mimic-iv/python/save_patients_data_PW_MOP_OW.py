...
feature_names = [
    ...
    "mv_non_vasive", "mv_oxygen_therapy", "mv_unknown", "mv_none", "race_w", "race_b", "race_l", "race_o"
]
    ...
    medians_data = measurements.groupby("stay_id").agg(
        {
            ...
            "mv_invasive": "first", "mv_non_vasive": "first", "mv_unknown": "first",
...
stay_ids = [30234310, 35444415, 34483718, 37265901, 32047172, 30605147, 34483718, 39837892, 36565128, 34483718, 35132513, 33548150]
merged_measurements = merged_measurements[merged_measurements["stay_id"].isin(stay_ids)]

merged_measurements = merged_measurements.rename(columns={"death_in_24_hours": "death"})
merged_measurements.to_csv(os.path.join(dst_dir, f"icustays_model_data_PW_{prediction_window}__MOP_{time}__OW_{outcome_window}.csv"), index=False)